...
module sui_bank::bank_tests { 
  ...
  use sui_bank::bank_tests::{Self, Bank, OwnerCap};
  ...
  fun test_deposit() {
    ...
    next_tx(scenario_mut, ALICE); 
    {
      ...
      assert_eq(bank_tests::admin_balance(&bank), 5);
      assert_eq(bank_tests::user_balance(&bank, ALICE), 95);
    ...
    next_tx(scenario_mut, ALICE); 
    {
      ...
      assert_eq(bank_tests::admin_balance(&bank), 7);
      assert_eq(bank_tests::user_balance(&bank, ALICE), 143);
  ...
  fun test_withdraw() {
    ...
    next_tx(scenario_mut, ALICE); 
    {
      ...
      let coin_out = bank_tests::withdraw(&mut bank, ctx(scenario_mut));
    ...
    next_tx(scenario_mut, ALICE); 
    {
      ...
      assert_eq(bank_tests::user_balance(&bank, ALICE), 0);
  ...
  fun test_partial_withdraw() {
    ...
    next_tx(scenario_mut, ALICE); 
    {
      ...
      let coin_out = bank_tests::partial_withdraw(&mut bank, 10, ctx(scenario_mut));

      assert_eq(burn_for_testing(coin_out), 10);
      assert_eq(bank_tests::user_balance(&bank, ALICE), 85);
  ...
  fun test_claim() {
    ...
    next_tx(scenario_mut, ADMIN); 
    {
      ...
      let coin_out = bank_tests::claim(&cap,&mut bank, ctx(scenario_mut));

      assert_eq(burn_for_testing(coin_out), 5);
      assert_eq(bank_tests::admin_balance(&bank), 0);
  ...
  #[test]
  #[expected_failure(abort_code = bank_tests::ENotEnoughBalance)]
  fun test_partial_withdraw_not_enough_balance() {
    ...
    next_tx(scenario_mut, ALICE); 
    {
      ...
      let coin_out = bank_tests::partial_withdraw(&mut bank, 1001, ctx(scenario_mut));
  ...
  fun init_bank(): Scenario {
   let scenario = test_scenario::begin(ADMIN);
    ...
    next_tx(scenario_mut, ADMIN);
    {
      bank_tests::init_for_testing(ctx(scenario_mut));
    };
  ...
  fun deposit(bank: &mut Bank, value: u64, scenario_mut: &mut Scenario) {
    bank_tests::deposit(bank, mint_for_testing(value, ctx(scenario_mut)), ctx(scenario_mut));
  }
}